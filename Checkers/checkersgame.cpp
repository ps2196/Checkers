...
void CheckersGame::calculateMovesForPiece(int row, int col)
{
    ...
    if( (color == WHITE && m_active_player == BLACK_PLAYER) ||
        ...
        return;
    QList<QPair<int, int>> availablePos;
...
void CheckersGame::calculateMovesForPiece(int row, int col, int piece,  QList<QPair<int, int>> &availablePos)
{
    ...
    if(piece == CheckersBoard::BLACK_PAWN || queen)
    {
        ...
        if(bottom_right == CheckersBoard::EMPTY_SQUARE)
            availablePos.push_back(QPair<int,int>(row+1, col-1));
    }
    //Bottom left square
    if(piece == CheckersBoard::BLACK_PAWN || queen)
    {
...
bool CheckersGame::capturesForPiece(int row, int col, int piece,  QList<QPair<int, int>> &availablePos)
{
    if(piece == CheckersBoard::EMPTY_SQUARE || piece == CheckersBoard::NOT_ON_BOARD ||
        _board->square(row,col) == CheckersBoard::NOT_ON_BOARD)
        return false;
